...
from sscape.utils_io import (
    pprint, config_pprint_logging, save_topic_model_snapshot, load_list_of_unicode_from_txt)
from utils_snapshots import (
    make_snapshot_path_for_lap, download_snapshot,
...
def get_best_idx_function_for_colname(score_name):
...
def select_best_from_many_runs(
    ...
    # Setup logging
    suffix = "__select_best__y=%s_score=%s_legend=%s" % (
            target_y_name,
    ...
    best_df = select_best_df_at_each_value_of_specific_vars(
        ...
        score_idx_func=get_best_idx_function_for_colname(selection_score_colname),
...
def load_df_from_all_folders_matching_list_of_patterns(
    ...
    for path_pattern in list_of_path_patterns:
        cur_alg_df = load_df_from_all_folders_that_match_pattern(
            ...
            task_ids=task_ids,
            column_names=column_names)
        cur_alg_df['N_STATES'] = cur_alg_df['N_STATES'].astype(np.int32)
        if query_str is not None:
            cur_alg_df = cur_alg_df.query(query_str).copy()

        if legend_name == 'BP_sLDA':
            print '*** adjusting loss_tr for BP_sLDA'
            cur_alg_df['LOSS_TR'] = \
                cur_alg_df['LOSS_TR'].values - cur_alg_df['LOSS_PI'].values
...
def select_best_df_at_each_value_of_specific_vars(
    ...
    print "Finding snapshots with %s of %s" % (
        score_idx_func.__name__, score_colname)
    print "Among snapshots satisfying query: %s" % query
    ...
    for query_str in query_str_list:
        best_job_df = make_best_job_df(
            ...
            score_idx_func=score_idx_func,
            target_splitname=target_splitname,
            **kwargs)
        if best_job_df is None:
            print "NO BEST TASK AVAILABLE FOR %s + %s" % (legend_name, query_str)
            continue
        ...
        for query_str in cur_queries:
            is_bad = False
            for no_leg_key in no_legend_keys:
                if query_str.count(no_leg_key) > 0:
                    is_bad = True
            cur_ulegend_name += " " + query_str.strip()
            if not is_bad:
                cur_legend_name += " " + query_str.strip()
    ...
    print "ON SPLIT %s:" % (target_splitname)
...
def make_best_job_df(
    ...
    for jj, job_path in enumerate(job_paths):
        ...
        if verbose:
            print job_path.split(os.path.sep)[-1]

        if cur_job_best_df.shape[0] < 1:
            if verbose:
                print 'Too small to satisfy query'
            continue

        split_name_chk = np.unique(cur_job_best_df['SPLIT_NAME'].values)
        if len(split_name_chk) != 1:
            print ">>>", split_name_chk
        ...
        best_score_idx[jj] = score_idx_func(
        ...
        best_lap_idx[jj] = cur_job_best_df['LAP'].values[
            best_score_idx[jj]]
        if verbose:
            print "%s = %.4f at lap %9.3f of task %s" % (
    ...
    if np.allclose(best_score, default_score):
        return None
    best_job_idx = score_idx_func(best_score)
    ...
    for job_path in job_paths:
        ...
        for tt, taskidstr in enumerate(taskids):
            ...
            if not np.all(np.isfinite(task_df[score_colname].values)):
                print task_df[score_colname].values
            best_score_idx[tt] = score_idx_func(task_df[score_colname].values)
            best_score[tt] = task_df[score_colname].values[best_score_idx[tt]]
        best_task_idx = score_idx_func(best_score)
...
def load_df_from_all_folders_that_match_pattern(
    ...
    for src_path in src_path_list:
        df = load_df_from_training_results_folder(
            src_path=src_path, 
            task_ids=task_ids,
            when_path_not_exists=when_path_not_exists,
            column_names=column_names,
            engine=engine,
...
def load_df_from_training_results_folder(
        src_path='',
        task_ids='1',
        when_path_not_exists='raise_error',
        column_names=COLUMN_NAMES,
        y_ind=0,
        engine=None,
        csv_pattern='callback_%s_info.csv',
    ...
    for task_id in task_ids:
        ...
        if not os.path.exists(task_path):
            if when_path_not_exists.count('error'):
        ...
        tr_csv_path = os.path.join(task_path, csv_pattern % 'train')
        tr_y_csv_path = os.path.join(task_path, csv_pattern % 'train_observe_y')
        va_csv_path = os.path.join(task_path, csv_pattern % 'valid')
        te_csv_path = os.path.join(task_path, csv_pattern % 'test')
        if os.path.exists(tr_csv_path): # and os.path.exists(va_csv_path):
            names = None
            try:
                tr_df = pd.read_csv(tr_csv_path, engine=engine)    
            except ValueError as e:
                if not str(e).count("Expected"):
                    raise e 
                # Fix things
                header = pd.read_csv(tr_csv_path, engine=engine, nrows=1)
                names = (
                    [c for c in header.columns[:14]] + ['N_STATES']
                    + [c for c in header.columns[14:]])
                def load_corrupted(corr_csv_path, names=names):
                    tr_df = pd.read_csv(
                        corr_csv_path, engine=engine, names=names, skiprows=1)
                    new_tr_df = tr_df.copy()
                    bad_rows = pd.isnull(tr_df[tr_df.columns[-1]].values)
                    for c in range(tr_df.shape[1] - 1, 14, -1):
                        new_tr_df.ix[bad_rows, c] = tr_df.ix[bad_rows, c-1]
                    new_tr_df.ix[bad_rows, 14] = new_tr_df.ix[tr_df.shape[0]-1, 14]
                    return new_tr_df

                tr_df = load_corrupted(tr_csv_path, names=names)

            try:
                if names is None:
                    va_df = pd.read_csv(va_csv_path, engine=engine)
                else:
                    va_df = load_corrupted(va_csv_path, names)
            except IOError:
                va_df = None
            try:
                if names is None:
                    te_df = pd.read_csv(te_csv_path, engine=engine)
                else:
                    te_df = load_corrupted(te_csv_path, names)
            except IOError:
                te_df = None
            try:
                if names is None:
                    tr_y_df = pd.read_csv(tr_y_csv_path, engine=engine)
                else:
                    tr_y_df = load_corrupted(tr_y_csv_path, names)
            except IOError:
                tr_y_df = None

            n_rows = tr_df.shape[0]
            if va_df is not None:
                n_rows = np.minimum(tr_df.shape[0], va_df.shape[0])
            if te_df is not None:
                n_rows = np.minimum(te_df.shape[0], n_rows)
                te_df = te_df[:n_rows]
            if va_df is not None:
                va_df = va_df[:n_rows]
            tr_df = tr_df[:n_rows]
            if tr_y_df is not None:
                tr_y_df = tr_y_df[:n_rows]

            row_dict_list = list()
            for prefix, df in [
                    ('TRAIN', tr_df), ('VALID', va_df), ('TEST', te_df),
                    ('TRAIN_OBSERVE_Y', tr_y_df)]:
                if df is None:
                    continue
                for _, row_df in df.iterrows():
                    row_dict = df_dict.copy()
                    row_dict['SPLIT_NAME'] = prefix
                    for key, val in row_df.to_dict().items():
                        key = key.strip().upper()
                        if key[-1].isdigit():
                            if not key.endswith('_%d' % y_ind):
                                continue
                            key = key[:key.rfind('_')]
                        row_dict[key] = val
                    row_dict_list.append(row_dict)
        ...
        """
        # DEPRECATED: OLD .txt CALLBACK FILES
        else:
            train_files = [s for s in sorted(glob.glob(
                os.path.join(task_path, 'callback_train*.txt')))]
            test_files = [s for s in sorted(glob.glob(
                os.path.join(task_path, 'callback_valid*.txt')))]
            if len(test_files) != len(train_files):
                raise ValueError(
                    "Task-specific train/test results do not match\n." +
                    task_path)

            row_dict_list = list()
            for ii, (train_path, test_path) in \
                    enumerate(zip(train_files, test_files)):
                basename = train_path.rstrip(
                ...
                fieldname = basename\
                    ...
                    .replace('train_', '')
                try:
                    # Skip files like roc_auc_1, roc_auc_2
                    int(fieldname.split("_")[-1])
                    continue
                except ValueError:
                    pass                
                train_arr = np.loadtxt(train_path)
                test_arr = np.loadtxt(test_path)

                if ii == 0:
                    for aa in xrange(train_arr.size):
                        row_dict_list.append(df_dict.copy())

                assert train_arr.size == test_arr.size
                for aa in xrange(train_arr.size):
                    train_key = 'TRAIN_' + fieldname.upper()
                    if train_key not in column_names:
                        break
                    row_dict_list[aa][train_key] \
                        = train_arr[aa]
                    row_dict_list[aa]['VALID_' + fieldname.upper()] \
                        = test_arr[aa]
        """
...
def simplify_best_df_and_make_unicode_friendly(
        best_df,
        replacements_ascii={'λ':'lam', '=':''},
        replacements={'WEIGHT_Y':'λ', '==':'=', "'":""},
        at_best_keys = ['LOSS_TR_AT_BEST_SNAPSHOT', 'LOSS_X_AT_BEST_SNAPSHOT', 'LOSS_Y_AT_BEST_SNAPSHOT'],
        ):
    ...
    for row_id in range(best_df.shape[0]):
        ...
        best_df.ix[row_id, 'LEGEND_NAME_ASCII'] = (' '.join(leg_str_ascii.decode('ascii').split())).replace(' ', '_')
    ...
    for leg in np.unique(best_df['_UNIQUE_LEGEND_NAME'].values):
        for split in np.unique(best_df['SPLIT_NAME'].values):
            leg_row_mask = np.logical_and(
                best_df._UNIQUE_LEGEND_NAME.values == leg,
                best_df.SPLIT_NAME.values == split)
            cur_row_mask = np.logical_and(at_best_row_mask, leg_row_mask)
            try:
                assert np.sum(cur_row_mask) == 1
            except AssertionError as e:
                print "leg", leg
                print "split", split
                print np.sum(cur_row_mask)
                print np.flatnonzero(cur_row_mask)
                raise e
            assert np.sum(cur_row_mask) < np.sum(leg_row_mask)
            for key in at_best_keys:
                val_at_best = float(best_df.ix[cur_row_mask, key.replace('_AT_BEST_SNAPSHOT', '')].values[0])
                best_df.ix[leg_row_mask, key] = val_at_best

    # Verify all row indices are unique
    assert best_df.shape[0] == np.unique(best_df.index.values).size

    print ''
    print '==== best_df created with these LEGEND_NAME values'
    for uval in np.unique(best_df['LEGEND_NAME'].values):
        print uval
    return best_df
